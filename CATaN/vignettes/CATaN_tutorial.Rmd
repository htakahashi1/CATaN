...
files    <- rownames(supp)
geo_file <- grep("raw_counts", files, value = TRUE)[1]
...
Note: `align_cc_to_snps()` loads all TF peaks into memory (~78 million peaks).
Parallel workers may each hold a copy, so start with `SerialParam()` and
increase workers only if memory allows.
